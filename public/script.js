...
async function updateKMBPositions(route) {
  ...
  data.data.forEach(bus => {
    if (bus.eta_seq === 1 && bus.eta) {
      let marker = L.marker([22.3 + Math.random() * 0.05, 114.17 + Math.random() * 0.05], { icon: busIcon })
        .addTo(map)
        .bindPopup(`巴士 ${route}<br>車牌: ${bus.license_plate || "N/A"}<br>型號: ${bus.bus_model || "N/A"}`);
...
async function updateCityBusPositions(company, route) {
  ...
  data.data.forEach(bus => {
    if (bus.eta) {
      let marker = L.marker([22.28 + Math.random() * 0.05, 114.16 + Math.random() * 0.05], { icon: busIcon })
        .addTo(map)
        .bindPopup(`巴士 ${route}<br>車牌: ${bus.license_plate || "N/A"}<br>型號: ${bus.bus_model || "N/A"}`);
      busMarkers.push(marker);
    }
